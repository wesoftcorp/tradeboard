...
from .iifl_mapping import IiflCapabilityRegistry, IiflExchangeMapper


class IiflWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Iifl XTS specific implementation of the WebSocket adapter"""
    ...
    def initialize(
        ...
        """
        Initialize connection with Iifl XTS WebSocket API

        Args:
            broker_name: Name of the broker (always 'iifl' in this case)
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError("Missing Iifl XTS API credentials in environment variables")
    ...
    def connect(self) -> None:
        """Establish connection to Iifl XTS WebSocket"""
    ...
    def disconnect(self) -> None:
        ...
        self.logger.info("*** DISCONNECT CALLED - Starting Iifl disconnect process ***")
    ...
    def subscribe(
        ...
        """
        Subscribe to market data with Iifl XTS specific implementation
        ...
        if mode == 3:  # Depth mode
            if not IiflCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                # If requested depth is not supported, use the highest available
                actual_depth = IiflCapabilityRegistry.get_fallback_depth_level(
        ...
        # Create instrument list for Iifl XTS API
    ...
    def _get_exchange_segment(self, exchange: str) -> str:
        ...
        """
        return IiflExchangeMapper.get_exchange_type(exchange)
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> dict[str, Any]:
        ...
        instruments = [
            {
                "exchangeSegment": IiflExchangeMapper.get_exchange_type(brexchange),
    ...
    def _on_open(self, wsapp) -> None:
        ...
        self.logger.info("Connected to Iifl XTS WebSocket")
        self.connected = True
    ...
    def _on_error(self, wsapp, error) -> None:
        ...
        self.logger.error(f"Iifl XTS WebSocket error: {error}")

    def _on_close(self, wsapp) -> None:
        """Callback when connection is closed"""
        self.logger.info("Iifl XTS WebSocket connection closed")
        self.connected = False
    ...
    def _on_data(self, wsapp, message) -> None:
        ...
        try:
            self.logger.info(f"RAW IIFL DATA: Type: {type(message)}, Data: {message}")
            self.logger.info(
