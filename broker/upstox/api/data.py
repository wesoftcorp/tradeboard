...
class BrokerData:
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if symbol in known_exchanges and exchange not in known_exchanges:
                ...
                logger.warning(
                    f"Detected reversed parameters: symbol='{symbol}', exchange='{exchange}'. Auto-correcting..."
                )
            ...
            if response.get("status") != "success":
                ...
                logger.exception(
                    f"Failed to get quotes for {instrument_key}: {full_error_msg} | Response: {response}"
                )
            ...
            for key, value in quote_data.items():
                logger.debug(
                    f"Checking key: {key}, instrument_token: {value.get('instrument_token')}"
                )
        ...
        for item in symbols:
            ...
            try:
                ...
                if not instrument_key:
                    logger.warning(
                        f"Skipping symbol {symbol} on {exchange}: could not resolve instrument key"
                    )
                ...
                key_map[instrument_key] = {"symbol": symbol, "exchange": exchange}
    ...
    def get_history(
        ...
        try:
            ...
            if not chunk_days:
                ...
                chunk_days = 30
                logger.warning(
                    f"Unknown interval {unit}/{interval_value}, using default {chunk_days} days"
                )
    ...
    def _fetch_chunk_data(
        ...
        try:
            ...
            if unit in ["minutes", "hours"] and end_date.date() == current_date.date():
                ...
                intraday_url = (
                    f"/historical-candle/intraday/{encoded_symbol}/{unit}/{interval_value}"
                )
                ...
                try:
                    ...
                    if intraday_response.get("status") == "success":
                        ...
                        filtered_candles = self._filter_candles_by_date(
                            intraday_candles, start_date, end_date
                        )
            ...
            if not all_candles or start_date.date() < current_date.date():
                ...
                try:
                    ...
                    if historical_response.get("status") == "success":
                        ...
                        logger.info(
                            f"Got {len(historical_candles)} candles from historical endpoint"
                        )
            ...
            logger.info(
                f"Checking daily logic: unit={unit}, interval_value={interval_value}, original_interval={interval}"
            )
            if unit == "days" and interval == "D":
                today = datetime.now().date()
                logger.info(
                    f"Daily timeframe check: today={today}, start_date={start_date.date()}, end_date={end_date.date()}"
                )
                if start_date.date() <= today <= end_date.date():
                    ...
                    if all_candles:
                        for candle in all_candles:
                            try:
                                candle_date = pd.to_datetime(
                                    candle[0],
                                    unit="ms" if isinstance(candle[0], (int, float)) else None,
                                ).date()
                        ...
                        try:
                            ...
                            if quotes and quotes.get("ltp", 0) > 0:
                                ...
                                if all_candles:
                                    ...
                                    if (
                                        last_open == quotes_open
                                        and last_high == quotes_high
                                        and last_low == quotes_low
                                        and last_close == quotes_close
                                        and last_volume == quotes_volume
                                    ):
                                        is_stale = True
                                        logger.warning(
                                            "Quotes data appears stale (identical to last candle). Skipping today's candle to avoid duplicates."
                                        )
                                        logger.warning(
                                            f"Last candle: O={last_open}, H={last_high}, L={last_low}, C={last_close}, V={last_volume}"
                                        )
                                        logger.warning(
                                            f"Quotes data: O={quotes_open}, H={quotes_high}, L={quotes_low}, C={quotes_close}, V={quotes_volume}"
                                        )
            ...
            df = pd.DataFrame(
                all_candles, columns=["timestamp", "open", "high", "low", "close", "volume", "oi"]
            )
    ...
    def _filter_candles_by_date(
        self, candles: list, start_date: datetime, end_date: datetime
    ) -> list:
        """
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            if response.get("status") != "success":
                ...
                logger.exception(
                    f"Failed to get market depth for {instrument_key}: {full_error_msg}"
                )
                raise Exception(full_error_msg)
