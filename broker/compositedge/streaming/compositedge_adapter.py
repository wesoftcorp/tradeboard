...
from .compositedge_mapping import CompositedgeCapabilityRegistry, CompositedgeExchangeMapper


class CompositedgeWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Compositedge XTS specific implementation of the WebSocket adapter"""
    ...
    def __init__(self):
        ...
        self.logger.info(f"Compositedge adapter initialized with ZMQ port: {self.zmq_port}")

    def initialize(
        self, broker_name: str, user_id: str, auth_data: dict[str, str] | None = None
    ) -> None:
        """
        Initialize connection with Compositedge XTS WebSocket API

        Args:
            broker_name: Name of the broker (always 'compositedge' in this case)
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError(
                    "Missing Compositedge XTS API credentials in environment variables"
                )
    ...
    def connect(self) -> None:
        """Establish connection to Compositedge XTS WebSocket"""
    ...
    def _connect_with_retry(self) -> None:
        ...
        try:
            while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
                ...
                try:
                    self.logger.info(
                        f"Connecting to Compositedge XTS WebSocket (attempt {self.reconnect_attempts + 1})"
                    )
                    client.connect()
    ...
    def subscribe(
        ...
        """
        Subscribe to market data with Compositedge XTS specific implementation
        ...
        if mode == 3:  # Depth mode
            if not CompositedgeCapabilityRegistry.is_depth_level_supported(exchange, depth_level):
                # If requested depth is not supported, use the highest available
                actual_depth = CompositedgeCapabilityRegistry.get_fallback_depth_level(
    ...
    def _get_exchange_segment(self, exchange: str) -> str:
        ...
        """
        return CompositedgeExchangeMapper.get_exchange_type(exchange)
    ...
    def unsubscribe(self, symbol: str, exchange: str, mode: int = 2) -> dict[str, Any]:
        ...
        instruments = [
            {
                "exchangeSegment": CompositedgeExchangeMapper.get_exchange_type(brexchange),
    ...
    def _on_error(self, wsapp, error) -> None:
        ...
        self.logger.error(f"Compositedge XTS WebSocket error: {error}")
