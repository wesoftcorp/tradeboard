...
def download_nubra_instruments(output_path):
    ...
    if not auth_token:
        raise Exception(
            f"No valid auth token found for user '{login_username}'. Please login first."
        )

    headers = {"Authorization": f"Bearer {auth_token}", "x-device-id": "TradeBoard"}
...
def process_nubra_json(path):
    ...
    df["exchange"] = df.apply(
        lambda r: (
            "NFO"
            if (r["exchange"] == "NSE" and r["derivative_type"] != "STOCK")
            else "BFO"
            if (r["exchange"] == "BSE" and r["derivative_type"] != "STOCK")
            else r["exchange"]
        ),
        axis=1,
    )
    ...
    expiry_dt = pd.to_datetime(
        expiry_series.fillna(0).astype("Int64").astype(str), format="%Y%m%d", errors="coerce"
    )
    ...
    df.loc[fut_mask, "symbol"] = df.loc[fut_mask, "asset"] + sym_expiry[fut_mask] + "FUT"
...
def process_nubra_indexes(path):
    ...
    df["exchange"] = df["brexchange"].apply(
        lambda x: "NSE_INDEX" if x == "NSE" else "BSE_INDEX" if x == "BSE" else x + "_INDEX"
    )
