...
* On pair success: `export_session()` → Fernet-encrypted → persisted to `whatsapp_config.session_blob` in `TradeBoard.db` → temp file unlinked
...
* `TradeBoard==1.0.49` → `TradeBoard==1.0.50`
...
4. Done. The bot auto-starts and reconnects on every server boot from the encrypted session in `TradeBoard.db`.
...
* **WhatsApp service docs**: <https://docs.TradeBoard.in/api-documentation/v1/whatsapp-services>
...
```
GET https://docs.TradeBoard.in/change-log/release/version-2.0.1.1-released.md?ask=<question>
```
