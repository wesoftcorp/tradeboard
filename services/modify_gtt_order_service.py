...
def modify_gtt_order_with_auth(
    ...
    message = (
        response_message.get("message", "Failed to modify GTT")
        if isinstance(response_message, dict)
...
def modify_gtt_order(
        ...
        if not get_analyze_mode():
            ...
            if user_id:
                ...
                if order_mode == "semi_auto":
                    error_response = {
                        "status": "error",
                        "message": "Modify GTT order is not allowed in Semi-Auto mode. Switch to Auto mode.",
                    }
    ...
    if auth_token and broker:
        return modify_gtt_order_with_auth(order_data, auth_token, broker, original_data)
